...
class Model(object):
    ...
    def __init__(self):
        ...
        t__ = time.time()
        self.setup_model()
        print(">setup_model ", time.time()-t__)

        t__ = time.time()
        self.load_data()
        print(">load_data ", time.time()-t__)
    ...
    def setup_model(self, input=None):
        ...
        fx = lasagne.nonlinearities.elu
        mx = 'max'
        #fx = lasagne.nonlinearities.rectify
        #mx = 'average_exc_pad'
        fx2 = fx
        ...
        net['conv1_1'] = ConvLayer(net['img'],     64, 3, pad=1, nonlinearity=fx)
        net['conv1_2'] = ConvLayer(net['conv1_1'], 64, 3, pad=1, nonlinearity=fx)
        #net['pool1']   = ConvLayer(net['conv1_2'], 128, 2, pad=1, stride=(2,2), nonlinearity=fx2)
        net['pool1']   = PoolLayer(net['conv1_2'], 2, mode=mx)
        net['conv2_1'] = ConvLayer(net['pool1'],   128, 3, pad=1, nonlinearity=fx)
        net['conv2_2'] = ConvLayer(net['conv2_1'], 128, 3, pad=1, nonlinearity=fx)
        net['pool2']   = PoolLayer(net['conv2_2'], 2, mode=mx)
        net['conv3_1'] = ConvLayer(net['pool2'],   256, 3, pad=1, nonlinearity=fx)
        net['conv3_2'] = ConvLayer(net['conv3_1'], 256, 3, pad=1, nonlinearity=fx)
        net['conv3_3'] = ConvLayer(net['conv3_2'], 256, 3, pad=1, nonlinearity=fx)
        net['conv3_4'] = ConvLayer(net['conv3_3'], 256, 3, pad=1, nonlinearity=fx)
        net['pool3']   = PoolLayer(net['conv3_4'], 2, mode=mx)
        net['conv4_1'] = ConvLayer(net['pool3'],   512, 3, pad=1, nonlinearity=fx)
        net['conv4_2'] = ConvLayer(net['conv4_1'], 512, 3, pad=1, nonlinearity=fx)
        net['conv4_3'] = ConvLayer(net['conv4_2'], 512, 3, pad=1, nonlinearity=fx)
        net['conv4_4'] = ConvLayer(net['conv4_3'], 512, 3, pad=1, nonlinearity=fx)
        net['pool4']   = PoolLayer(net['conv4_4'], 2, mode=mx)
        net['conv5_1'] = ConvLayer(net['pool4'],   512, 3, pad=1, nonlinearity=fx)
        net['conv5_2'] = ConvLayer(net['conv5_1'], 512, 3, pad=1, nonlinearity=fx)
        net['conv5_3'] = ConvLayer(net['conv5_2'], 512, 3, pad=1, nonlinearity=fx)
        net['conv5_4'] = ConvLayer(net['conv5_3'], 512, 3, pad=1, nonlinearity=fx)
        ...
        for j, i in itertools.product(range(5), range(4)):
            ...
            if i == 0:
                ...
                print("net['map%i']"%(j+1), " = PoolLayer(net['map'], %i)"%(2**j))
            ...
            if args.semantic_weight > 0.0:
                print("net['dup%s']"%suffix, " = InputLayer(net['sem%s'])"%suffix)
            else:
                print("net['dup%s']"%suffix, " = InputLayer(net['conv%s'])"%suffix)
            net['nn'+suffix] = ConvLayer(net['dup'+suffix], 1, 3, b=None, pad=0, flip_filters=False)
            print("net['nn%s']"%suffix, " = ConvLayer(net['dup%s'], 1, 3, b=None, pad=0, flip_filters=False)"%suffix)
    ...
    def load_data(self):
        ...
        print(">p.load ", time.time()-t__)

        #params = lasagne.layers.get_all_param_values(self.network['main'])
        #paramCount = len(params)

        paramCount = lasagne.layers.count_params(self.network['main'])
        lasagne.layers.set_all_param_values(self.network['main'], data[:paramCount])
...
class NeuralGenerator(object):
    ...
    def __init__(self):
        ...
        print(">NeuralGenerator ", time.time()-self.start_time)
    ...
    def do_extract_patches(self, layers, size=3, stride=1):
        ...
        for l, f in layers:
            ...
            #print ("patches: [", l, " - ", f, ": ", f.shape, "] ", patches.shape)
            #print ("patches ", l, T.printing.debugprint(f), T.printing.debugprint(patches))
            #print ("patches: [" + f.shape + "] " + patches.shape)

        print(">do_extract_patches", time.time()-t__)
        return results
    ...
    def run(self):
        ...
        for i in range(args.phases):
            ...
            if args.seed == 'previous':
                ...
                Xn = Xn.transpose((2, 0, 1))[np.newaxis]                                                    # ??? transpose
            ...
            data_bounds = np.zeros((np.product(Xn.shape), 2), dtype=np.float64)                             # ??? float64
